...
btn_where = KeyboardButton('/where')
btn_friends = KeyboardButton('/friends')
btn_token = KeyboardButton('/token')
btn_where_me = KeyboardButton('/where_me')
btn_help = KeyboardButton('/help')

mainMenu = ReplyKeyboardMarkup(resize_keyboard=True).add(btn_where, btn_friends, btn_token, btn_where_me, btn_help)
...
async def command_start(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, f'Привет! Отправьте сюда свое местоположение или livelocation, '
                                                 f' потом наберите команду /where', reply_markup=mainMenu)
...
async def command_token(message):
    ...
    await bot.send_message(message.from_user.id, f"отправьте этот токен своему другу: {new_token}",
...
def get_friends(id):
    global dict_of_friends
    return list(set([val[1] for key, val in dict_of_friends.items() if val[0] == id] + [val[0] for key, val in
                                                                                        dict_of_friends.items() if
                                                                                        val[1] == id]))
...
async def command_help(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,
                           'Команды: \n/token - создает код подтверждения\n/friends - список друзей\n/where - основная команда, показывающая расстояние до отслеживаемых людей.\n\nНажмите reply/ответить на сообщение с вашей локацией, введите название места, где вы находитесь',
                           reply_markup=mainMenu)
    print(message, "HELP")


@dp.message_handler(commands=['where_me'])
async def command_where_me(message):
    cur_user = return_user(message)
    if cur_user.current_location:
        await bot.send_message(message.from_user.id,
                               f"ближайшая к вам точка: {nearest_point(cur_user) or 'неизвестное место'}",
                               reply_markup=mainMenu)
    else:
        await bot.send_message(message.from_user.id,
                               "мы не можем определить место, так как вы не прислали свою геолокацию: нажмите /help чтобы прочитать инструкцию",
                               reply_markup=mainMenu)
    ...
    for user in users.values():
        if user.id in get_friends(cur_user.id):
            if user.current_location:
                if cur_user.current_location:
                    where_vse.append(
                        [get_dist(cur_user.current_location.current_pos, user.current_location.current_pos),
                         user.username,
                         get_time(message.date - user.current_location.time_stamp), nearest_point(user)])


                else:
                    await bot.send_message(message.from_user.id,
                                           "мы не можем определить расстояние, так как вы не прислали свою геолокацию",
                                           reply_markup=mainMenu)

            else:
                stroka += f"@{user.username} нет активной геолокации\n"

    for dist, name, last_update, name_of_loc in sorted(where_vse):
...
async def get_location(message: types.Message):
    ...
    user = return_user(message)
    a = NewPoint(message, user)

    loc = message.location

    location_coords[(loc.latitude, loc.longitude)] = [None, user.id, message.message_id,
                                                      message.forward_date or message.edit_date or message.date,
                                                      message.date]
    sql_add_location(loc.latitude, loc.longitude, user.id, message.message_id, name=None,
                     time_stamp=message.forward_date or message.edit_date or message.date, message_date=message.date)

    if a.live_period:  # если у нас лайв.локейшн
        if a.kod not in a.user.tracks:  # если новый лайв.локейшн
            a.user.tracks[a.kod] = NewTrack(a)
        else:
...
def get_time(time):
    ...
    if sec < 60:
        ...
        return [sec // 3600, "часов"]
    elif sec < 604800:
        return [sec // 86400, "дней"]
    else:
        return [sec // 604800, "недель"]
...
def return_user(message):
    if users.get(message.from_user.id):
        ...
        message_dict = {'username': message.from_user.username, 'id': message.from_user.id,
                        'first_name': message.from_user.first_name, 'last_name': message.from_user.last_name}
        usr = NewUser(message_dict)
        users[usr.id] = usr
...
async def bot_message(message: types.Message):
    ...
    if availability_check:
        ...
        if message.from_user.id == friend.id:
            await bot.send_message(message.from_user.id, 'вы не можете добавить себя в отслеживаемые',
                                   reply_markup=mainMenu)

        elif (friend.id, message.from_user.id, False) in [val for code, val in dict_of_friends.items() if not val[2]]:
            await bot.send_message(message.from_user.id, 'вы уже в отслеживаемых с этим человеком',
                                   reply_markup=mainMenu)
